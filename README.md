...
### v1.1.0 (2026-03-19)
- **新增「重置進度」功能**：在介面上新增了「🔄 重置進度」按鈕，可以清除所有資料夾的整理記錄。
- **強化「重新命名」**：現在會記住上次的命名，並防止輸入無效的檔案名稱。
- **改進「進度儲存」**：開啟資料夾時，程式會詢問是否要「從上次中斷處繼續」。
- **優化「錯誤處理」**：當遇到無法讀取的損毀圖片時，會顯示警告並自動跳過。
- **更即時的儲存**：每次操作（保留/刪除）後都會立即儲存進度。
- **介面更新**：程式標題現在會顯示版本號。
